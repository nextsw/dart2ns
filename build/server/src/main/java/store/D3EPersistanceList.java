...
public class D3EPersistanceList<E> extends ArrayList<E> {
	...
	public static <E> D3EPersistanceList<E> primitive(DBObject master, int field) {
		return new D3EPersistanceList<>(master, field);
	}
	
	public static <E> D3EPersistanceList<E> reference(DBObject master, int field) {
		return new D3EPersistanceList<>(master, field);
	}
	
	public static <E> D3EPersistanceList<E> child(DBObject master, int field) {
		D3EPersistanceList<E> list = new D3EPersistanceList<>(master, field);
		list.child = true;
		return list;
	}
	
	public static <E> D3EPersistanceList<E> inverse(DBObject master, int field) {
		D3EPersistanceList<E> list = new D3EPersistanceList<>(master, field);
		list.inverse = true;
		return list;
	}
	...
	public void _unproxy(List result) {
		this.proxy = false;
		if(child) {
			result.forEach(this::updateMaster);
		}
		super.addAll(result);
	}
	...
	public void setAll(List<E> nums) {
		...
		super.addAll(nums);
		if(child) {
			nums.forEach(this::updateMaster);
		}
	}
	
	private void updateMaster(Object e) {
		DBObject o = (DBObject) e;
		o._setMasterObject(master);
		o._setChildIdx(field);
		o._updateChanges();
	}
}
