...
public class D3EEntityManagerProvider {
	...
	private class EntityManagerImpl implements IEntityManager {
		...
		public <T> T getById(int type, long id) {
			...
			D3ELogger.debug("By Id: type: " + type + ", id: " + id + " , " + query);
		...
		public <T> List<T> findAll(int type) {
			DModel<?> dm = schema.getType(type);
			StringBuilder sb = new StringBuilder();
			sb.append("select ").append(queryBuilder.createRefColumn(dm, "_id")).append(" from ")
					.append(dm.getTableName());
			String query = sb.toString();
			D3ELogger.debug("Find All: type: " + type + " , " + query);
		...
		private void execute(D3EQuery query) {
			...
			if (query.query != null) {
				...
				D3ELogger.debug("Insert/Update: " + q);
		...
		public void unproxy(DatabaseObject obj) {
			...
			D3ELogger.debug("Unproxy Object: " + obj.getId() + " : " + query);
			List<DatabaseObject> list = jdbcTemplate.getJdbcTemplate().query(query,
					new SingleObjectMapper(cache, obj, type, selectedFields));
			if (list.isEmpty()) {
		...
		public void unproxyCollection(D3EPersistanceList<?> list) {
			...
			D3ELogger.debug("Unproxy Collection: " + master.getId() + ", " + field.getName() + " : " + query);
		...
		public void unproxyDFile(DFile file) {
			...
			D3ELogger.debug("Unproxy DFile: " + file.getId() + " : " + query);
