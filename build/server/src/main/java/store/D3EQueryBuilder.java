...
public class D3EQueryBuilder {
	...
	private DFile checkAndSaveDFile(D3EQuery query, DFile df) {
		if (!resourceHandler.isSaved(df)) {
			...
			D3EQuery createDFile = generateCreateDFileQuery(df);
			query.addPreQuery(createDFile);
		}
		return df;
	}
				...
				if (ref.getIndex() == SchemaConstants.DFile) {
					...
					if (field.isChild()) {
						...
						if (!ch.oldValues.containsKey(field.getIndex()) || oldChild == newChild) {
							query.addNextQuery(generateUpdateQuery(field.getReference(), newChild));
							continue;
						}
						if (newChild != null) {
					...
					if (old != null) {
						...
						for (Object v : values) {
							if (oldCopy.contains(v)) {
								D3EQuery chq = generateUpdateQuery(field.getReference(), (DatabaseObject) v);
								query.addNextQuery(chq);
								oldCopy.remove(v);
							} else {
								D3EQuery chq = generateCreateQuery(field.getReference(), (DatabaseObject) v);
								query.addPreQuery(chq);
						...
						if (!oldCopy.isEmpty()) {
							D3EQuery chq = generateMultiDeleteQuery(oldCopy);
							query.addPreQuery(chq);
						}
					} else {
						for (Object v : values) {
	...
	public String generateSelectAllQuery(DModel type, List<RowField> selectedFields, long id) {
		...
		ICustomFieldProcessor processor = CustomFieldService.get().getProcessor(type.getType());
		if (processor != null) {
			processor.selectAll(sb, type, id, selectedFields, joins, ag, alias);
		}
		...
		for (DField df : fields) {
			...
			case Reference:
				...
				if (ref.isDocument()) {
					...
					sb.append(", ");
					sb.append(createRefColumn(df.getReference(), alias + "." + embeddedPrefix + df.getColumnName()));
					selectedFields.add(new RowField(df));
				}
				break;
			case PrimitiveCollection:
	...
	public String generateSelectCollectionQuery(DModel<?> type, DField<?, ?> field, long id) {
		...
		switch (field.getType()) {
		case InverseCollection:
			b.append(createRefColumn(field.getReference(), "_id"));
			break;
		case PrimitiveCollection:
			b.append(field.getColumnName());
			break;
		case ReferenceCollection:
			b.append(createRefColumn(field.getReference(), field.getColumnName()));
			break;
		default:
			break;
		}
	...
	public String createRefColumn(DModel<?> ref, String clm) {
		if (ref.getIndex() == SchemaConstants.DFile) {
			return clm;
		} else {
			...
			if (types.length > 1) {
				...
				return clm + ", type_of" + mp.getTableName() + "(" + clm + ")";
			} else {
