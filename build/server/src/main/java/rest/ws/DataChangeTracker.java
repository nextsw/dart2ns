...
public class DataChangeTracker implements Runnable {
	...
	class TypeListener implements Disposable {
		Set<Integer> types = new HashSet<>();
		boolean disposed;
		TriConsumer<DBObject, DBObject, StoreEventType> listener;
		BitSet fields;
		int type;

		TypeListener(int type, BitSet fields, TriConsumer<DBObject, DBObject, StoreEventType> listener) {
			this.listener = listener;
			this.type = type;
			this.fields = fields;
		}
	...
	@PostConstruct
	public void init() {
		new Thread(this, "Change Tracker").start();
	}
		...
		while (!shutdown) {
			try {
				...
				case Scan: {
					...
					scan(null, null, event.obj, event.field, event.dl, null, null, true, MapExt.Map());
					emProvider.clear();
					break;
				}
				case DisposeTl:
	...
	private void scan(DBObject parent, DField parentField, Object obj, Field field, DisposableListener dl,
			ObjectsToSend toSend, List<Integer> allParents, boolean sendAll, Map<DBObject, DBChange> changes) {
		int type = 0;
		long id = 0;
		DBObject dbObj = null;
		if (obj instanceof OutObject) {
		...
		if (toSend != null && dbObj != null) {
			...
			if (sendAll) {
				...
				DBChange change = changes.get(dbObj);
...
//		 + LISTEXT.MAP(SEL.GETFIELDS(), (F) -> F.GETField().getName()) + " Object Type: " + objType.getType() + " Type : " + type);
	...
	private void remove(DBObject obj, ObjectUsage ou) {
		...
		// D3ELogger.info("Un Watching " + sel.getType().getType() + " ID: " + id + "
		// Fields: "
	...
	public <T> T fromTypeAndId(TypeAndId ti) {
	...
	public <T> List<T> fromTypeAndId(List<TypeAndId> list) {
		List<T> res = new ArrayList<>();
		list.forEach(a -> res.add(fromTypeAndId(a)));
		return res;
	}

	public Disposable listen(int type, BitSet fields, TriConsumer<DBObject, DBObject, StoreEventType> listener) {
	...
	public void fire(DBObject object, StoreEventType changeType) {
		...
		changes.put(object, prepareChanges(object));
		if (object instanceof DatabaseObject) {
			((DatabaseObject) object).visitChildren(a -> changes.put(a, prepareChanges(a)));
		}
		object._clearChanges();
		event.changes = changes;
		eventQueue.add(event);
	}

	private DBChange prepareChanges(DBObject object) {
		DBChange ch = object._changes();
		if(object instanceof DatabaseObject) {
			DatabaseObject db = ((DatabaseObject) object);
				...
				if (fieldType == FieldType.Reference) {
					...
					if (_newValue != null) {
						...
						for (ObjectUsage ou : interests.refListeners) {
							...
							Iterable<Field> where = IterableExt.where(expand, (i) -> i.getField() == dField);
							for (Field f : where) {
								scan(object, dField, dbObj, f, ou.listener, toSend, allParents, oldId != dbObj.getId(),
										changes);
							}
						}
					}
					...
					boolean childChange = _oldValue == null;
		...
		for (ObjectListener ol : listeners) {
			if (ol.listener instanceof DisposableListener) {
				...
				try {
					...
					tl.listener.accept(object, old, changeType);
				}catch (Exception e) {
	...
	public void fire(List<DataStoreEvent> changes) {
		...
		for (DataStoreEvent ds : changes) {
			...
			if (object instanceof DBObject) {
				dbChanges.put((DBObject) object,  prepareChanges((DBObject)object));
			}
			if (object instanceof DatabaseObject) {
				((DatabaseObject) object).visitChildren(a -> dbChanges.put(a, prepareChanges(a)));
			}
		}
